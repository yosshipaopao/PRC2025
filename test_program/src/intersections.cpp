...
Intersections::Intersections(Sensors &s, Motors &m)
    : sensors(s), motors(m), crossDetected(false), leftTDetected(false),
      allActiveTurnLeft(false), leftTCount(0),
...
void Intersections::updateFlags()
{
    ...
    // 各交差パターンの検知判定
    bool leftT = farLeft && left && midLeft && !midRight && !right;  // 左T字路
    ...
    if (leftT)
    {
        Debug::print("LEFT_T_SENSOR_ACTIVE : ");
        Debug::print(" leftTCount=");
        Debug::println(leftTCount);
    }
    if (allActive)
    {
        Debug::print("ALL_ACTIVE_SENSOR_ACTIVE");
        Debug::print(" allActiveCount=");
        Debug::println(allActiveCount);
    }
    ...
    if (farLeft)
        crossSensorFlags[0] = now;
    if (left)
        crossSensorFlags[1] = now;
    if (midLeft)
        crossSensorFlags[2] = now;
    if (center)
        crossSensorFlags[3] = now;
    if (midRight)
        crossSensorFlags[4] = now;
    if (right)
        crossSensorFlags[5] = now;
    ...
    if (!crossDetected)
    {
        ...
        for (int i = 1; i < 6; i++)
        {
            ...
            if (now - crossSensorFlags[i] > Config::CROSS_DETECT_WINDOW_MS)
            {
    ...
    if (leftTCount >= Config::INTERSECTION_STABLE_COUNT)
    {
        leftTDetected = true;
    ...
    if ((!crossDetected && !leftTDetected) && (allActiveCount >= Config::INTERSECTION_STABLE_COUNT))
    {
        ...
        allActiveDetectedMs = now;
    }
    // 5個センサー検知中の処理
    if ((!crossDetected && !leftTDetected) && (now - allActiveDetectedMs < Config::ALL_ACTIVE_TIMEOUT_MS))
    {
        // 全センサーオフで左折トリガー
...
void Intersections::turnLeftUntilLine(float &lastError)
{
    ...
    motors.set(-Config::LEFT_TURN_SPEED, Config::LEFT_TURN_SPEED);
    {
        ...
        while (sensors.isActiveLineDetected(Config::LINE_CENTER_INDEX))
        {
            ...
            {
                Debug::println("WARN: turnLeft step2 timeout");
                break;
            }
    ...
    motors.set(-Config::LEFT_TURN_SPEED_SLOW, Config::LEFT_TURN_SPEED_SLOW);
    {
        ...
        while (!sensors.isActiveLineDetected(Config::LINE_CENTER_INDEX))
        {
            if (millis() - turnStart > TURN_TIMEOUT_MS)
            {
                Debug::println("WARN: turnLeft step3 timeout");
                break;
            }
            Debug::sensorStates(lastError);
            delay(5);
